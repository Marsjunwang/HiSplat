...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.test_cfg.compute_scores:
            ...
            # If PoseNet predictions are available (from enhancer), compute pose metrics
            try:
                if isinstance(result_dict, dict) and "enhancer_outputs" in result_dict:
                    eo = result_dict["enhancer_outputs"]
                    ...
                    if pred_01 is not None and gt_01 is not None:
                        ...
                        # Initialize pose metric lists
                        if f"pose_rot_deg" not in self.test_step_outputs:
                            self.test_step_outputs[f"pose_rot_deg"] = []
                        if f"pose_trans" not in self.test_step_outputs:
                            self.test_step_outputs[f"pose_trans"] = []
                        self.test_step_outputs[f"pose_rot_deg"].append(float(rot_deg.item()))
                        self.test_step_outputs[f"pose_trans"].append(float(trans_l2.item()))
                        # Optionally dump per-scene pose results
                        try:
                            pose_log_path = path / scene / "pose.txt"
                            os.makedirs(str(path / scene), exist_ok=True)
                            with open(pose_log_path, "w") as f:
                                f.write(f"rot_deg: {rot_deg.item():.4f}\n")
                                f.write(f"trans: {trans_l2.item():.4f}\n")
                        except Exception:
                            pass
            except Exception:
                pass
            ...
            try:
                ...
                if hasattr(self.encoder, "enhancer") and getattr(self.encoder, "enhancer") is not None:
                    ...
                    if (
                        ...
                        and enhancer.pose_enhancer.cfg.input_data != "image"
                    ):
